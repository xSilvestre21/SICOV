...
import { Plus, Search, ChevronLeft, ChevronRight, Truck } from 'lucide-react';
...
export function SuppliersListPage() {
  ...
  const setPage = (p) => { const params = new URLSearchParams(searchParams); params.set('page', p); setSearchParams(params); };
  const handleSearch = (e) => { e.preventDefault(); const fd = new FormData(e.target); const params = new URLSearchParams(searchParams); params.set('search', fd.get('search')); params.set('page', '1'); setSearchParams(params); };
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <form onSubmit={handleSearch} className="flex-1 flex gap-2">
            <div className="relative flex-1">
              ...
              <input name="search" defaultValue={search} placeholder="Buscar por nome, CNPJ, cidade..." className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-[#b0b087] focus:border-[#58706d] focus:ring-1 focus:ring-[#58706d] outline-none" />
            </div>
            <Button type="submit" variant="secondary" size="md"><Search size={14} /></Button>
          </form>
          <div className="flex gap-2">
            {['', 'true', 'false'].map((v) => (
              <button key={v} onClick={() => { const p = new URLSearchParams(searchParams); if (v) p.set('active', v); else p.delete('active'); p.set('page', '1'); setSearchParams(p); }}
                className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${active === v ? 'bg-[#58706d] text-white border-[#58706d]' : 'bg-white text-[#4b5757] border-[#e3e3d1] hover:border-[#58706d]'}`}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <Button variant="ghost" size="sm" disabled={page <= 1} onClick={() => setPage(page - 1)}><ChevronLeft size={16} /></Button>
          <span className="text-sm text-[#4b5757]">Página {page} de {totalPages}</span>
          <Button variant="ghost" size="sm" disabled={page >= totalPages} onClick={() => setPage(page + 1)}><ChevronRight size={16} /></Button>
        </div>
      )}
    </div>
  );
